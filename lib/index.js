...
import { appendFile, lstat, mkdir, open, readFile, readdir, readlink, realpath, rename, rm, stat, unlink, writeFile } from "node:fs/promises";
...
async function openIndex(dbPath) {
	...
	if (userVersion > 3) {
		closeQuietly(db);
		throw new Error(`${dbPath} was written by a newer version of Turnscope (schema ${userVersion}; this build understands 3)`);
	}
	...
	const migrate = () => {
		...
		try {
			for (let version = current; version < 3; version += 1) {
...
function createRepository(handle) {
	...
	*/
	...
	function parseVerdictJson(raw) {
		const parsed = JSON.parse(raw);
		if (parsed === null || typeof parsed !== "object" || Array.isArray(parsed)) throw new Error(`recovery_plans.verdict_json: expected an object, read ${raw}`);
		return parsed;
	}
	/** Throw if a stored operations array is not an array of operations. */
	function parseOperationsJson(raw) {
		const parsed = JSON.parse(raw);
		if (!Array.isArray(parsed)) throw new Error(`recovery_plans.operations_json: expected a JSON array, read ${raw}`);
		return parsed;
	}
	function parseJournalOperationJson(raw) {
		const parsed = JSON.parse(raw);
		if (parsed === null || typeof parsed !== "object" || Array.isArray(parsed)) throw new Error(`recovery_journal.operation_json: expected an object, read ${raw}`);
		return parsed;
	}
	function parseJournalErrorJson(raw) {
		const parsed = JSON.parse(raw);
		if (parsed === null || typeof parsed !== "object" || Array.isArray(parsed)) throw new Error(`recovery_journal.error_json: expected an object, read ${raw}`);
		return parsed;
	}
	const toRecoveryPlan = (row) => ({
		schemaVersion: 3,
		id: text(row, "id"),
		turnId: text(row, "turn_id"),
	...
	const toRecoveryJournalEntry = (row) => ({
		schemaVersion: 3,
		...
		occurredAt: integer(row, "occurred_at"),
		...absent("error", row["error_json"] === null ? void 0 : parseJournalErrorJson(text(row, "error_json")))
	});
	const RECOVERY_PLAN_COLUMNS = "id, turn_id, verdict_json, evaluation_id, state_hash, operations_json, before_checkpoint_id, status, created_at, expires_at";
	const RECOVERY_JOURNAL_COLUMNS = "plan_id, seq, state, operation_json, occurred_at, error_json";
	const putRecoveryPlan = async (record) => {
		statement(`INSERT INTO recovery_plans (${RECOVERY_PLAN_COLUMNS})
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
       ON CONFLICT(id) DO UPDATE SET
         turn_id = excluded.turn_id,
         ...
         status = excluded.status,
         created_at = excluded.created_at,
         expires_at = excluded.expires_at`).run(record.id, record.turnId, JSON.stringify(record.verdict), record.evaluationId, record.stateHash, JSON.stringify(record.operations), record.beforeCheckpointId, record.status, record.createdAt, record.expiresAt);
	};
	const getRecoveryPlan = async (id) => {
		const row = one(`SELECT ${RECOVERY_PLAN_COLUMNS} FROM recovery_plans WHERE id = ?`, id);
		return row === void 0 ? void 0 : toRecoveryPlan(row);
	};
	const listRecoveryPlans = async (turnId) => {
		return all(`SELECT ${RECOVERY_PLAN_COLUMNS} FROM recovery_plans
       WHERE turn_id = ?
       ORDER BY created_at DESC, id ASC`, turnId).map(toRecoveryPlan);
	};
	const updateRecoveryPlanStatus = async (id, status) => {
		statement("UPDATE recovery_plans SET status = ? WHERE id = ?").run(status, id);
	};
	const putRecoveryJournalEntry = async (record) => {
		statement(`INSERT INTO recovery_journal (${RECOVERY_JOURNAL_COLUMNS})
       VALUES (?, ?, ?, ?, ?, ?)
       ON CONFLICT(plan_id, seq, state) DO NOTHING`).run(record.planId, record.seq, record.state, JSON.stringify(record.operation), record.occurredAt, nullable(record.error === void 0 ? void 0 : JSON.stringify(record.error)));
	};
	const listRecoveryJournal = async (planId) => {
		return all(`SELECT ${RECOVERY_JOURNAL_COLUMNS} FROM recovery_journal
       WHERE plan_id = ?
       ORDER BY seq ASC, occurred_at ASC, state ASC`, planId).map(toRecoveryJournalEntry);
	};
	const listUnfinishedRecoveryPlans = async (now) => {
		return all(`SELECT ${RECOVERY_PLAN_COLUMNS} FROM recovery_plans
       WHERE status = 'applying'
          OR (status = 'previewed' AND expires_at < ?)
       ORDER BY created_at ASC`, now).map(toRecoveryPlan);
	};
...
function createGitPort(runner) {
	...
	*/
	const blobAt = async (cwd, treeish, path) => runGit(cwd, [
		"cat-file",
		"blob",
		`${treeish}:${path}`
	]);
...
const DEFAULT_DIFF_OPTIONS = Object.freeze({
	contextLines: 3,
	maxLines: 2e3,
	maxEdits: 1024
});
...
function splitLines(bytes) {
	if (bytes.subarray(0, 8e3).includes(0)) return void 0;
...
const context = (text, before, after) => ({
	kind: "context",
	text,
	beforeIndex: before,
	afterIndex: after
...
*/
function unifiedDiff(before, after, options = DEFAULT_DIFF_OPTIONS) {
...
*/
function editScript(before, after, maxEdits) {
	let head = 0;
	...
	const middle = myers(before.slice(head, before.length - tail), after.slice(head, after.length - tail), maxEdits);
	const list = [];
	let beforeIndex = 0;
	let afterIndex = 0;
	for (const step of [
...
const replaceAll = (before, after) => [...before.map((text) => ({
	kind: "remove",
	text
})), ...after.map((text) => ({
	kind: "add",
...
function myers(before, after, maxEdits) {
	...
	for (let d = 0; d <= limit && found < 0; d += 1) {
		...
		for (let k = -d; k <= d; k += 2) {
			let x;
			if (k === -d || k !== d && furthest[offset + k - 1] < furthest[offset + k + 1]) x = furthest[offset + k + 1];
			else x = furthest[offset + k - 1] + 1;
			let y = x - k;
	...
	return {
		list: backtrack(before, after, trace, found, offset),
		truncated: false
	};
}
/** Walk the trace backwards into the script, then read it forwards. */
function backtrack(before, after, trace, distance, offset) {
	const reversed = [];
	let x = before.length;
	let y = after.length;
...
* does this.
*/
function groupChanges(ops, contextLines) {
	const groups = [];
	let start = -1;
	let lastChange = -1;
...
function toHunk(slice) {
	const lines = slice.map((op) => op.kind === "context" ? {
		kind: op.kind,
		text: op.text,
		beforeLine: op.beforeIndex + 1,
		afterLine: op.afterIndex + 1
	} : op.kind === "remove" ? {
		kind: op.kind,
		text: op.text,
		beforeLine: op.beforeIndex + 1
...
//#endregion
//#region src/host/diff/reader.ts
/**
...
* the agent wrote it.
*/
function createFileDiffReader(deps) {
	const options = {
		...DEFAULT_DIFF_OPTIONS,
		...deps.limits ?? {}
	};
	const read = async (turn, workspace, path) => {
		const change = (await deps.sink.listFileChanges(turn.id)).find((candidate) => candidate.path === path);
		if (change === void 0) return void 0;
		const pre = await checkpointOf(turn.id, "pre");
		const post = await checkpointOf(turn.id, "post");
		if (pre === void 0 || post === void 0) return failed(change, void 0, void 0, void 0, "no-checkpoint", "The turn does not have both a before and an after checkpoint, so there is nothing to compare.");
		...
		if (before.kind === "unavailable") return failed(change, beforeRow, afterRow, sides, before.reason, before.detail);
		if (after.kind === "unavailable") return failed(change, beforeRow, afterRow, sides, after.reason, after.detail);
		const beforeText = textOf(before, beforeRow);
		const afterText = textOf(after, afterRow);
		if (beforeText === void 0 || afterText === void 0) return {
	...
	const checkpointOf = async (turnId, phase) => {
		const record = await deps.sink.getCheckpoint(checkpointIdFor(turnId, phase));
		if (record === void 0) return void 0;
		return {
			record,
			paths: await deps.sink.listCheckpointPaths(record.id)
		};
	};
	/**
	...
	*/
	const resolveBefore = async (change, pre, row, workspace) => {
		if (change.kind === "created") return { kind: "absent" };
		if (row !== void 0) return fromRow(row, "before");
		if (pre.headOid === void 0) return {
			kind: "unavailable",
			reason: "git-unavailable",
			detail: "The file was unchanged when the turn started, and the commit it started from could not be read."
		};
		const bytes = await deps.git.blobAt(workspace.repoRoot, pre.headOid, change.previousPath ?? change.path);
		if (bytes === void 0) return {
			kind: "unavailable",
			reason: "git-unavailable",
			detail: "The file was unchanged when the turn started, and its committed content is no longer in the repository."
		};
		return {
			kind: "bytes",
			bytes,
	...
	const resolveAfter = async (change, row) => {
	...
	const fromRow = async (row, when) => {
		if (row.blobRef === void 0) return {
			kind: "unavailable",
			reason: "not-recorded",
			detail: `The content ${when === "before" ? "before" : "after"} this turn was fingerprinted but not saved: it was larger than the size a checkpoint copies.`
		};
		if (!await deps.store.has(row.blobRef)) return {
			kind: "unavailable",
			reason: "missing-blob",
		...
		try {
			return {
				kind: "bytes",
				bytes: await deps.store.get(row.blobRef),
...
const header = (change, beforeRow, afterRow) => {
	const status = afterRow?.status ?? beforeRow?.status;
	return {
		path: change.path,
		...change.previousPath === void 0 ? {} : { previousPath: change.previousPath },
		kind: change.kind,
...
* about this".
*/
const failed = (change, beforeRow, afterRow, sides, reason, detail) => ({
	...header(change, beforeRow, afterRow),
...
const emptySide = (source, contentHash) => ({
	source,
	byteSize: 0,
	lineCount: 0,
...
const sideInfo = (resolved, contentHash) => {
	...
	return {
		...
		lineCount: text === void 0 ? 0 : text.lines.length,
...
*/
const textOf = (resolved, row) => {
	if (resolved.kind === "absent") return {
		lines: [],
		endsWithNewline: false
	};
	if (resolved.kind === "unavailable") return void 0;
	if (row?.binary === true) return void 0;
...
function fromPostOnly(ctx, after, now) {
	if (after.status === "clean") return void 0;
	const created = after.status === "added" || after.status === "untracked";
	const kind = created ? "created" : after.status === "renamed" ? "renamed" : after.present ? "modified" : "deleted";
...
function createQueryService(deps) {
	...
	return {
		...
		getDiff: async (request) => {
			...
			return lookup(diff === void 0 ? void 0 : { diff });
		}
	};
}
...
//#endregion
//#region src/host/recovery/hash.ts
/**
...
*/
function computeStateHash(input) {
	const enc = canonicalize(input);
	return `sha256:${createHash("sha256").update(enc, "utf8").digest("hex")}`;
}
//#endregion
//#region src/host/recovery/planner.ts
/**
...
*/
function buildRelevantPaths(ops, snapshots) {
	const out = [];
	for (const op of ops) {
		...
		out.push({
			path: op.path,
			contentHash: snap.currentContentHash,
			staged: snap.staged
		});
	}
	return out;
...
*   `noop unchanged`.
*/
function classifyPath(change, snapshot) {
	if (change === void 0) return {
		kind: "noop",
		path: snapshot.path,
		reason: "unchanged"
	};
	if (change.attribution === "BASELINE") return {
		kind: "noop",
		path: snapshot.path,
		reason: "baseline_only"
	};
	if (change.attribution === "DRIFT") return {
		kind: "noop",
		path: snapshot.path,
		reason: "drift_preserved"
	};
	if (change.attribution === "UNCERTAIN") return {
		kind: "noop",
		path: snapshot.path,
		reason: "unknown"
	};
	switch (change.kind) {
		...
		case "deleted":
			if (snapshot.beforeBlobRef === void 0) return {
				kind: "noop",
				path: snapshot.path,
				reason: "unknown"
			};
		...
		case "modified":
		case "binary_changed":
			if (snapshot.beforeBlobRef === void 0 || snapshot.afterBlobRef === void 0) return {
				kind: "noop",
				path: snapshot.path,
				reason: "unknown"
			};
...
//#endregion
//#region src/host/recovery/runner/paths.ts
/**
...
function resolveDryRunRoot(recoveryRoot, planId) {
	if (planId.length === 0) throw new Error("planId must be non-empty to resolve a dry-run directory");
	return join(recoveryRoot, DRYRUN_DIRNAME, planId);
}
/** The journal file for one plan, rooted at the recovery root. */
function resolveJournalPath(recoveryRoot, planId) {
	if (planId.length === 0) throw new Error("planId must be non-empty to resolve a journal file");
	return join(recoveryRoot, JOURNAL_DIRNAME, `${planId}.jsonl`);
}
//#endregion
//#region src/host/recovery/runner/journal.ts
/**
...
*/
async function appendJournal(recoveryRoot, entry) {
...
*/
async function deleteJournal(recoveryRoot, planId) {
	const path = resolveJournalPath(recoveryRoot, planId);
	await rename(path, `${path}.bak`).catch(() => void 0);
}
//#endregion
//#region src/host/recovery/patch/apply.ts
/**
...
function parsePatch(patch) {
	...
	if (i < lines.length && lines[i] === "Binary files differ") return {
		hunks: [],
		binary: true
	};
	const hunks = [];
	while (i < lines.length) {
		...
		const beforeCount = m[2] === void 0 ? 1 : Number(m[2]);
		const afterStart = Number(m[3]);
		const afterCount = m[4] === void 0 ? 1 : Number(m[4]);
		const body = [];
		while (i < lines.length) {
			const raw = lines[i];
			...
			if (raw === "\\ No newline at end of file") {
				...
				if (prev !== void 0) body[body.length - 1] = {
					kind: prev.kind,
					text: prev.text,
					noNewline: true
				};
			...
			if (raw.startsWith(" ")) body.push({
				kind: "context",
				text: raw.slice(1),
				noNewline: false
			});
			else if (raw.startsWith("-")) body.push({
				kind: "remove",
				text: raw.slice(1),
				noNewline: false
			});
...
function splitWithCrlf(bytes) {
	...
	return {
		lines: (trailingNewline ? text.slice(0, -1) : text).split("\n").map((line) => line.endsWith("\r") ? line.slice(0, -1) : line),
		trailingNewline
	};
}
function joinLines(lines, trailingNewline) {
	if (lines.length === 0) return trailingNewline ? new Uint8Array([10]) : /* @__PURE__ */ new Uint8Array();
	const sep = "\n";
...
function applyPatch(input) {
	...
	for (let h = 0; h < parsed.hunks.length; h += 1) {
		...
		const hunkStart = hunk.beforeStart - 1;
		if (hunkStart < cursor) return {
			kind: "conflict",
			path: input.path,
			reason: "hunk-offset"
		};
		...
		while (bodyCursor < hunk.body.length) {
			...
			if (line.kind === "context") {
				...
				const search = (start, step) => {
					for (let i = start; step > 0 ? i < start + 20 * step && i < lines.length && i >= 0 : i > start + 20 * step && i >= 0; i += step) if (lines[i] === line.text) return i;
					return -1;
				};
				const nextMatch = search(appliedAt, 1);
				if (nextMatch >= 0) {
					for (let i = appliedAt; i < nextMatch; i += 1) result.push(lines[i] ?? "");
					result.push(line.text);
				...
				if (search(appliedAt, -1) >= 0) return {
					kind: "conflict",
					path: input.path,
					reason: "hunk-offset"
				};
			...
			if (line.kind === "remove") {
				if (lines[appliedAt] !== line.text) return {
					kind: "conflict",
					path: input.path,
					reason: "context-mismatch"
				};
	...
	return {
		kind: "ok",
		resultBytes: joinLines(result, split.trailingNewline)
	};
}
//#endregion
//#region src/host/recovery/patch/reverse-derive.ts
/**
...
*/
function renderUnified(path, hunks, reverse) {
...
*/
function reverseProjection(hunks) {
	return hunks.map((h) => ({
...
function reverseDerive(input) {
	const { path, beforeBytes, afterBytes } = input;
	if (beforeBytes === null && afterBytes === null) return {
		forwardText: "",
		reverseText: "",
		forwardHunkCount: 0,
		forwardHunks: []
	...
	if (beforeBytes === null) {
		const afterText = splitLines(afterBytes) ?? {
			lines: [],
			endsWithNewline: true
		};
		const forwardHunks = afterText.lines.length === 0 ? [] : [{
			beforeStart: 0,
			beforeCount: 0,
			afterStart: 1,
			afterCount: afterText.lines.length,
			lines: afterText.lines.map((t) => ({
				kind: "add",
				text: t,
				afterLine: 1
			}))
	...
	if (afterBytes === null) {
		const beforeText = splitLines(beforeBytes) ?? {
			lines: [],
			endsWithNewline: true
		};
		const forwardHunks = beforeText.lines.length === 0 ? [] : [{
			beforeStart: 1,
	...
	if (before === void 0 || after === void 0) {
		...
		const forwardHunks = totalLines === 0 ? [] : [{
			beforeStart: 1,
...
//#endregion
//#region src/host/recovery/runner/dryrun.ts
/**
...
*/
async function runDryRun(plan, objectStore, live, recoveryRoot) {
	...
	for (const op of plan.operations) {
		...
		if (op.kind === "restore") {
			...
			const currentBytes = await live.readCurrent(op.path) ?? /* @__PURE__ */ new Uint8Array();
			const { reverseText } = reverseDerive({
				path: op.path,
				beforeBytes,
				afterBytes
			});
...
//#endregion
//#region src/host/recovery/runner/apply.ts
/**
...
async function atomicWrite(target, bytes) {
	...
	const fh = await open(tempPath, "w", 384);
	try {
...
async function runApply(input) {
	...
	for (const op of plan.operations) {
		if (op.kind === "noop") continue;
		if (op.kind === "restore" || op.kind === "recreate_deleted_file") await objectStore.stat(op.targetBlobRef);
	}
	...
	for (const op of plan.operations) {
		...
		if (op.kind === "restore") {
			...
			if (reverseText.length > 0) {
				...
				if (r.kind === "conflict") {
					journal.push(makeEntry(plan.id, clock, op, "rolled_back", {
						code: "PATCH_CONFLICT",
						message: r.reason
					}));
		...
		const backupPath = join(resolveDryRunRoot(homeDir, plan.id), ".backup", op.path);
		...
		if (liveHash !== expectedHash) {
			if (existedBefore) await rename(backupPath, target);
			else await unlink(target).catch(() => void 0);
			journal.push(makeEntry(plan.id, clock, op, "rolled_back", {
				code: "VERIFY_MISMATCH",
				message: `expected ${expectedHash}, got ${liveHash}`
			}));
	...
	for (const entry of journal) await appendJournal(homeDir, entry);
	if (!ranRollback) await deleteJournal(homeDir, plan.id).catch(() => void 0);
	await unlink(join(resolveDryRunRoot(homeDir, plan.id), ".backup")).catch(() => void 0);
	const status = ranRollback ? "rolled_back" : failures.length === 0 ? "completed" : "failed";
	const result = {
		planId: plan.id,
		status,
...
function makeEntry(planId, clock, op, state, error) {
	return {
		schemaVersion: 3,
		planId,
		seq: clock.nextSeq(),
...
//#endregion
//#region src/host/recovery/service.ts
/**
...
function createRecoveryService(deps) {
	...
	*/
	const collectPlannerInputs = async (turnId) => {
		...
		const first = (await sink.listCheckpoints(turnId))[0];
		if (first === void 0) return void 0;
		return {
			turn,
			workspace,
			fileChanges,
			checkpointPaths: await sink.listCheckpointPaths(first.id),
			beforeCheckpointId: first.id
		};
	};
	/**
	...
	*/
	const snapshotPaths = async (paths, workspace) => {
		return Promise.all(paths.map(async (path) => {
			const existsNow = await worktree.hashCurrent(workspace.repoRoot, path) !== void 0;
			return {
				path,
				staged: false,
				currentContentHash: existsNow ? await worktree.hashCurrent(workspace.repoRoot, path) : void 0,
				beforeBlobRef: void 0,
				afterBlobRef: void 0,
	...
	const liveFor = (workspaceRoot) => ({
		async hashCurrent(path) {
	...
	const dryrunLiveFor = (workspaceRoot) => ({ readCurrent: async (path) => {
		return await worktree.readCurrent(workspaceRoot, path) ?? null;
	} });
	return {
		previewRewind: async (request) => {
			const inputs = await collectPlannerInputs(request.turnId);
			if (inputs === void 0) return envelope({ failureReason: "turn or workspace not found, or turn has no checkpoints" });
			...
			if (!("level" in verdictOrError)) return envelope({ failureReason: verdictOrError.message });
			...
			const dryrun = await runDryRun(plan, store, dryrunLiveFor(workspace.repoRoot), homeDir);
			if (dryrun.ops.some((op) => op.kind === "conflict")) return envelope({
				plan,
				failureReason: dryrun.ops.filter((op) => op.kind === "conflict").map((op) => `${op.op.path}: ${op.reason}`).join("; ")
			});
			const expiresAt = clock.nowMs() + (request.ttlMs ?? 6e4);
			const previewed = {
				...plan,
				status: "previewed",
				expiresAt
		...
		listUnfinished: async (request) => {
			return envelope({
				plans: await sink.listUnfinishedRecoveryPlans(clock.nowMs()),
				...request.includeFinished === true ? {} : {}
			});
		}
	};
}
async function loadVerdictOrFail(sink, turn) {
	const verdict = await sink.getLatestVerdict(turn.id);
	if (verdict === void 0) return { message: "no verdict recorded for this turn" };
	return verdict;
}
//#endregion
//#region src/host/storage/worktree-reader.ts
/**
...
*/
const createNodeWorktreeReader = () => ({
	async hashCurrent(workspaceRoot, path) {
		try {
	...
	async readCurrent(workspaceRoot, path) {
		try {
...
var TurnscopeRemoteService = class extends TypertRemoteService {
	...
	recovery;
	constructor(ctx, query, recovery) {
...
function requireRequest(value, fields) {
	...
	const request = value;
	if (request["apiVersion"] !== 3) throw new Error(`${REMOTE_NAMESPACE}: client apiVersion ${String(request["apiVersion"])} does not match host 3`);
...
function requireGetDiffRequest(value) {
	return requireRequest(value, ["turnId", "path"]);
}
function requirePreviewRewindRequest(value) {
	const request = requireRequest(value, ["turnId", "evaluationId"]);
	const ttl = request.ttlMs;
	if (ttl !== void 0 && typeof ttl !== "number") throw new Error(`${REMOTE_NAMESPACE}: request field "ttlMs" must be a number`);
	return request;
}
function requireApplyRewindRequest(value) {
	return requireRequest(value, ["planId"]);
}
function requireListRecoveryPlansRequest(value) {
	const request = requireRequest(value, []);
	const include = request.includeFinished;
	if (include !== void 0 && typeof include !== "boolean") throw new Error(`${REMOTE_NAMESPACE}: request field "includeFinished" must be a boolean`);
	return request;
}
/**
...
*/
function mountTurnscopeRemoteWhenReady(ctx, query, recovery, diagnostics) {
...
*/
function mountTurnscopeRemote(ctx, query, recovery, diagnostics) {
	try {
