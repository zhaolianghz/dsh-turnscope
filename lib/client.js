window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		...
		function TurnDetailView({ state, now, t }) {
			switch (state.kind) {
				...
				case "missing": return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
					className: "turnscope-detail-note",
				...
				case "loaded": return /* @__PURE__ */ (0, react_jsx_runtime.jsx)(Loaded, {
					detail: state.detail,
					now,
					t
				});
		...
		*/
		function useTurnDetails(host, generation) {
			...
			const toggle = (0, react.useCallback)((turnId) => {
				setExpanded((current) => {
			...
			(0, react.useEffect)(() => {
				...
				for (const turnId of pending) host.getTurnDetail({
					apiVersion: 2,
					turnId
				}).then((reply) => {
		...
		const claim = (setCache, generation, turnIds) => {
			setCache((current) => {
		...
		const settle = (setCache, generation, turnId, state) => {
			setCache((current) => {
				const next = new Map(current.generation === generation ? current.states : NOTHING);
		...
		const read = (reply) => {
			switch (reply.kind) {
				...
				case "absent": return { kind: "missing" };
				case "unusable": return {
					kind: "failed",
					reason: reply.detail
				};
