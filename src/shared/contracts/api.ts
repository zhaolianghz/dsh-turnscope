 ...
 * exist.
 */
export const REMOTE_NAMESPACE = 'turnscope'

/** The endpoint name one method is reached by, e.g. `turnscope/listTurns`. */
export const endpointFor = (method: string): string => `${REMOTE_NAMESPACE}/${method}`
...
export const envelope = <T>(data: T): TurnscopeApiEnvelope<T> => ({
...
export const lookup = <T>(data: T | undefined): TurnscopeLookupReply<T> =>
  envelope<T | null>(data ?? null)
 ...
 */
export function readReply<T>(reply: unknown): ReplyRead<T> {
  if (typeof reply !== 'object' || reply === null) {
    return unusable('the host reply is not an object')
  }
  const candidate = reply as { apiVersion?: unknown; data?: unknown }
  if (candidate.apiVersion !== API_VERSION) {
    const host = String(candidate.apiVersion)
    return unusable(`the host answers contract version ${host}, this bundle speaks ${API_VERSION}`)
  }
  if (candidate.data === null) return { kind: 'absent' }
  if (candidate.data === undefined) return unusable('the host reply carries no data')
  return { kind: 'value', value: candidate.data as T }
}

const unusable = (detail: string): ReplyRead<never> => ({ kind: 'unusable', detail })
