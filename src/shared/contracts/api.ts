 ...
 */
export function unwrap<T>(reply: unknown): TurnscopeApiEnvelope<T> | undefined {
  if (typeof reply !== 'object' || reply === null) return undefined
  const candidate = reply as Partial<TurnscopeApiEnvelope<T>>
  if (candidate.apiVersion !== API_VERSION) return undefined
  if (!('data' in candidate)) return undefined
  return { apiVersion: candidate.apiVersion, data: candidate.data as T }
}
