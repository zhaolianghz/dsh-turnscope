...
export function freshnessOf(
  local: readonly TurnModel[],
  recorded: RecordedTurns | undefined,
): Freshness {
  if (recorded === undefined) return 'loading'
  if (recorded.problem !== undefined) return 'error'
  if (local.some(turn => !recorded.turns.has(turn.turn))) return 'stale'
  const running = local.length === 0
    ? [...recorded.turns.values()].some(turn => turn.status === 'running')
    : local.some(turn => turn.status === 'running')
  return running ? 'live' : 'stable'
}
